...
def run_training():
  ...
  with tf.Graph().as_default():
    ...
    image_batch_eval, label_batch_eval = inputs(train=False, batch_size=200,
                               num_epochs=FLAGS.num_epochs) #epoch should be??
    ...
    evalLogits = NetStructureV2.inference(image_batch_eval,
                             FLAGS.hidden1,
                             FLAGS.hidden2)
    ...
    # Evaluate model
    correct_pred = tf.equal(tf.argmax(evalLogits, 1), tf.cast(label_batch_eval, tf.int64))
        ...
        while True: #train until OutOfRangeError
          ...
          if step % 100 == 0:
            print('Step %d: loss = %.2f (%.3f sec)' % (step, loss_value,
                                                     duration), end='')
            print('The accuracy is now: %.4f' %(acc))
          step += 1
